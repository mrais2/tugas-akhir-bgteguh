...
String devstr =  "Monitoring Smoke Detector";
int max_frames = 150;
...
#define I2C_SDA 15
#define I2C_SCL 14
TwoWire I2CBus = TwoWire(0); // I2C untuk ESP32-CAM

// ======================= SENSOR CHANNELS ====================
#define MQ2_CHANNEL      0   // AIN0
#define MQ135_CHANNEL    1   // AIN1
#define FLAME_CHANNEL    2   // AIN2
...
// ======================= GAS CURVES =========================
float SmokeCurve_MQ2[3] = {2.3, 0.53, -0.44};
float SmokeCurve_MQ135[3] = {2.3, 0.42, -0.38};
...
char ssid[] = "Mrais";     // your network SSID (name)
char password[] = "25121999"; // your WiFi network password
// https://sites.google.com/a/usapiens.com/opnode/time-zones  -- find your timezone here
String TIMEZONE = "WIB-7";//your timezone

 
String chat_id = "1493469379";//your telegram ID
#define BOTtoken "7995568426:AAGuMs0EViR9_Fb0A8jRc6JVvzGUh7PpF6k"  // your Bot Token (Get from Botfather)
...
// int relay1 = 15;
// int relay2 = 13;
float MQRead(uint8_t channel);
float MQResistanceCalculation(int adcVal);
float MQGetGasPercentage(float rs_ro_ratio, float *curve);
...
// volatile bool active_interupt = false;
bool alarm_enabled = false;
bool alarmState = false; // Status alarm (ON/OFF)
...
if (text == "/1") { // Nyalakan alarm
    alarmState = true;
    digitalWrite(ALARM_PIN, HIGH);
    bot.sendMessage(chat_id, "Alarm diaktifkan", "");
}

if (text == "/0") { // Matikan alarm
    alarmState = false;
    ...
    if (text == "/status") {
      ...
      float rs_ro_mq2 = MQRead(MQ2_CHANNEL) / Ro_MQ2;
      float rs_ro_mq135 = MQRead(MQ135_CHANNEL) / Ro_MQ135;
      float smoke_mq2_ppm = MQGetGasPercentage(rs_ro_mq2, SmokeCurve_MQ2);
      float smoke_mq135_ppm = MQGetGasPercentage(rs_ro_mq135, SmokeCurve_MQ135);
      int adcValueFlame = readADC(FLAME_CHANNEL);
      float voltageFlame = ads.computeVolts(adcValueFlame);
...
String stat = "```"
              ...
              "Sensor Readings:\n"
              "MQ2    : " + String(smoke_mq2_ppm, 2) + " ppm\n"
              "MQ135  : " + String(smoke_mq135_ppm, 2) + " ppm\n"
              "Flame  : " + String(voltageFlame, 3) + " V\n"
              "Alarm  : " + String(digitalRead(ALARM_PIN) ? "ON" : "OFF") + "\n";

// Tambahkan setting lain ke dalam blok sebelum penutup ```

if (frame_interval == 0) {
  stat += "Speed  : Fast 3 sec\n";
} else if (frame_interval == 125) {
  stat += "Speed  : Med 10 sec\n";
} else {
  stat += "Speed  : Slow 40 sec\n";
}

stat += "Quality: " + String(quality) + "\n";
stat += "```"; // ⬅️ Tutup blok Markdown di akhir
...
bot.sendMessage(chat_id, stat, "Markdown");
    ...
    // }
    if (text == "/enable") {
    alarm_enabled = true; // Bukan alarmState
    bot.sendMessage(chat_id, "Sistem deteksi bahaya AKTIF", "");
    }

    if (text == "/disable") {
    alarm_enabled = false;
    // digitalWrite(ALARM_PIN, LOW); // Matikan alarm saat disable
}
    ...
    if (text == "/start") {
      ...
      welcome += "\n Configure the clip\n";
      welcome += "/enable: enable alrm\n";
      welcome += "/disable: disable alrm\n";
      ...
      welcome += "\n/1: alarmPin\n";
      // welcome += "/2: stringer\n";
      welcome += "/0: stringer & alarmPin OFF\n";
...
const int alarmPin = ALARM_PIN;

static void setupinterrupts() {
 
  pinMode(alarmPin, INPUT) ; //INPUT_PULLDOWN);
 
  Serial.print("Setup alarmPin = ");
  for (int i = 0; i < 10; i++) {
    Serial.print( digitalRead(alarmPin) ); Serial.print(", ");
    delay(100);
  }
  Serial.println(" ");
  
  esp_err_t err = gpio_isr_handler_add(static_cast<gpio_num_t>(alarmPin), &PIR_ISR, nullptr);

  if (err != ESP_OK) {
    Serial.printf("gpio_isr_handler_add failed (%x)", err);
  }
  gpio_set_intr_type(static_cast<gpio_num_t>(alarmPin), GPIO_INTR_POSEDGE);
...
//     // Serial.print("PIR Interupt>> "); Serial.println(digitalRead(alarmPin));

//     if (!active_interupt && alarm_enabled) {
...
//  static void IRAM_ATTR PIR_ISR(void* arg) {
//     if (digitalRead(alarmPin) == HIGH && alarm_enabled) {
//         // 1. Trigger recording
...
void setup() {
  ...
  if (!ads.begin(0x48, &I2CBus)) {
    Serial.println("ADS1115 not found. Check I2C connection!");
    while (1);
  }
...
void checkSensors() {
  ...
  String flameStatus;
  if (voltageFlame < 1.0) {
    flameStatus = "** Close Fire **";
  } else if (voltageFlame < 2.5) {
    flameStatus = "** Distant Fire **";
  } else {
    flameStatus = "No Fire";
  }
  ...
  Serial.print(smoke_mq2_ppm); Serial.print(" ppm | ");

  Serial.print("SMOKE (MQ135): ");
  Serial.print(smoke_mq135_ppm); Serial.print(" ppm | ");

  Serial.print("FLAME: ");
  Serial.print(voltageFlame, 3); Serial.print(" V => ");
  Serial.println(flameStatus);

  // Alarm logic
  bool smokeDanger = (smoke_mq2_ppm > 300 || smoke_mq135_ppm > 300);
  bool smokeWarning = (!smokeDanger && (smoke_mq2_ppm >= 200 || smoke_mq135_ppm >= 200));
  bool fireClose = voltageFlame < 1.0;
  bool fireDistant = (!fireClose && voltageFlame < 2.5);

  if (alarm_enabled && (smokeDanger || fireClose)) {
    digitalWrite(ALARM_PIN, HIGH);  // Continuous alarmPin
    // Send alert to Telegram
    String alertMsg = "🚨 DANGER ALERT 🚨\n";
    if (smokeDanger) alertMsg += "High smoke detected: " + String(max(smoke_mq2_ppm, smoke_mq135_ppm)) + " ppm\n";
    if (fireClose) alertMsg += "Fire detected close by!\n";
    alertMsg += "Immediate attention required!";
    ...
    // }
      ...
      if (vid_fb) {
        picture_ready = true;
        send_the_picture();  // Kirim foto
        send_the_video();    // Kirim video
      }
  } 
  else if (smokeWarning || fireDistant) {
    // Blinking alarmPin
    ...
    digitalWrite(ALARM_PIN, LOW);   // Safe
  }
}
...
float MQRead(uint8_t channel) {
  ...
  for (int i = 0; i < READ_SAMPLE_TIMES; i++) {
    rs += MQResistanceCalculation(readADC(channel));
...
float MQResistanceCalculation(int adcVal) {
  ...
  float rs = ((5.0 - voltage) * RL_VALUE) / voltage;
  return rs;
}
...
void send_the_picture() {
  if (digitalRead(ALARM_PIN) != HIGH) {
    Serial.println("Alarm not active - photo not sent");
    return;
  }

  digitalWrite(33, LOW);  // Turn on indicator LED
  ...
  // Sensor readings
  // float mq2_ppm = MQRead(MQ2_CHANNEL);
  // float mq135_ppm = MQRead(MQ135_CHANNEL);
  // int adcValueFlame = readADC(FLAME_CHANNEL);
  // float flameVoltage = ads.computeVolts(adcValueFlame);

  Serial.println(">>>>> Sending alarm photo (" + String(fb_length) + " bytes)");

  // Prepare caption
  // String caption = "🚨 *ALARM ACTIVATED!*\n"
  //                  "📊 *Sensor Values:*\n"
  //                  "- MQ2   : " + String(mq2_ppm, 2) + " ppm\n"
  //                  "- MQ135 : " + String(mq135_ppm, 2) + " ppm\n"
  //                  "- Flame : " + String(flameVoltage, 3) + " V";

  // String sent = bot.sendMultipartFormDataToTelegramWithCaption(
  //   "sendPhoto", 
  //   "photo", 
  //   "alarm_photo.jpg",
  //   "image/jpeg", 
  ...
  esp_camera_fb_return(vid_fb);
  digitalWrite(33, HIGH);  // Turn off indicator LED

  // Optional reset interrupt if needed
  // if (!avi_enabled) active_interupt = false;
}

void send_the_video() {
  if (digitalRead(ALARM_PIN) != HIGH) {
    Serial.println("Alarm not active - video not sent");
    return;
  }
  ...
  Serial.println(">>>>> Sending alarm video (" + String(avi_len) + " bytes)");
  ...
  localtime_r(&now, &timeinfo);
  char filename[50];
  strftime(filename, sizeof(filename), "alarm_%Y-%m-%d_%H.%M.%S.avi", &timeinfo);

  // Prepare caption
  // String caption = "⚠️ *ALARM RECORDING*\n"
  //                  "📊 *Sensor Values:*\n"
  //                  "- MQ2   : " + String(mq2_ppm, 2) + " ppm\n"
  //                  "- MQ135 : " + String(mq135_ppm, 2) + " ppm\n"
  //                  "- Flame : " + String(flameVoltage, 3) + " V";
  ...
  digitalWrite(33, HIGH);  // Turn off indicator LED
  bot.longPoll = 5;
  // active_interupt = false;  // Optional reset
}
